"""100% Pure CUDA Vectorized Boundary & Limits Sweet Spot Optimizer.

Executes all 50,000+ evaluations entirely in GPU VRAM (RTX 3060) with ZERO CPU bottlenecks.
Vectorizes:
  - Multi-tier dynamic exits in (B, M, T) tensors
  - Session window masks directly on CUDA
  - Day PnL aggregation via torch.scatter_add_
  - Cumulative drawdown and Calmar ratio via torch.cummax / torch.cumsum
  - Daily Win Rate (% Green Days) directly on CUDA tensors
"""
...
print("100% PURE CUDA VECTORIZED BOUNDARY & LIMITS OPTIMIZER (RTX 3060)", flush=True)
print(f"CUDA Device: {torch.cuda.get_device_name(0)} | Precision: TensorFloat32", flush=True)
print("=" * 135, flush=True)
...
# Pre-calculate month indices on CUDA for monthly win rate vectorization
...
def run_pure_gpu_sweep(session_name: str, start_bar: int, end_bar: int, mid_skip_s: int, mid_skip_e: int, geom_grid: list[dict], batch_size: int = 25):
    # Filter active mask on CUDA
    ...
    for i in range(0, len(geom_grid), batch_size):
        chunk = geom_grid[i: i + batch_size]
        B = len(chunk)
        ...
        # 2. Cumulative Equity & Max Drawdown on CUDA
        ...
        # Transfer only final summary metrics to CPU in 1 batch
...
def main():
    # Build Fine-Grained Geometry Parameter Grid
    ...
    for sl, t1t, t1l, t2t, t2l, tr, tp in itertools.product(initial_sls, t1_trigs, t1_locks, t2_trigs, t2_locks, trails, tps):
        if t1l >= t1t or t2l >= t2t or t1l >= t2l or t2l > tp:
            continue
        geom_grid.append({
    ...
    print(f"Generated {len(geom_grid):,} Valid Multi-Tier Geometry Configurations\n", flush=True)

    # 4 Core Session Boundaries
    sessions = [
    ...
    all_results = []
    total_evals = len(geom_grid) * len(sessions)
    print(f"Launching Pure GPU Sweep across {total_evals:,} Total Evaluations on RTX 3060...", flush=True)

    t0 = time.time()
    for s_name, s_start, s_end, m_start, m_end in sessions:
        t_s = time.time()
        print(f">>> Running Pure GPU Sweep: [{s_name}] ({len(geom_grid):,} configs)...", flush=True)
        res = run_pure_gpu_sweep(s_name, s_start, s_end, m_start, m_end, geom_grid, batch_size=25)
        all_results.extend(res)
        print(f"    Completed in {time.time()-t_s:.2f}s ({len(res):,} evals | {len(res)/(time.time()-t_s):.1f} configs/sec)", flush=True)

    total_time = time.time() - t0
    print("\n" + "=" * 145, flush=True)
    print(f"SWEEP COMPLETED in {total_time:.2f}s ({len(all_results):,} evaluations | {len(all_results)/total_time:.1f} configs/sec)", flush=True)
    print("=" * 145, flush=True)

    df = pd.DataFrame(all_results)

    # 1. Global Pareto Champion: Highest Calmar with >65% Daily WR & >Rs 25L Profit
    top_pareto_pool = df[(df["daily_win_rate"] >= 65.0) & (df["month_win_rate"] >= 90.0) & (df["net_rs"] >= 2000000.0)]
    ...
    # 2. Maximum Profit Champion (< Rs 15k Drawdown)
    top_profit_pool = df[df["max_drawdown"] <= 15000.0]
    champ_profit = (top_profit_pool.sort_values(by="net_rs", ascending=False).iloc[0].to_dict()
                    if len(top_profit_pool) > 0 else df.sort_values(by="net_rs", ascending=False).iloc[0].to_dict())
    ...
    print("\n--- TOP 3 SWEET SPOT CHAMPIONS ---", flush=True)
    for title, c in [
        ("1. GLOBAL SWEET SPOT CHAMPION (Highest Calmar + >65% Daily WR + >Rs 25L Profit)", champ_pareto),
        ("2. MAXIMUM NET PROFIT CHAMPION (Highest Profit with < Rs 15k Drawdown)", champ_profit),
        ("3. DAILY INCOME CHAMPION (Highest Daily Win Rate % Green Days)", champ_daily),
    ]:
        print(f"\n{title}:", flush=True)
        print(f"  * Session:                    {c['session']}", flush=True)
    ...
    # Save to JSON
    out_file = ROOT / "artifacts" / "f6_hybrid" / "pure_gpu_limits_champions.json"
    ...
    print(f"\n[Saved Pure GPU Limits Champions JSON]: {out_file}", flush=True)
