"""Master Undisputed Rejection Champion — 2020 to 2026 Strict 7-Year Backtest (GPU Accelerated).

Date Range: 2020-01-01 to 2026-08-23 (Exact 7 Years / ~1,588 Trading Days)
Engine: Master Undisputed Rejection Champion (Score >= 50, Two-Bar Confirmation, 15m Trend Gate)
"""
...
print("=" * 135)
print("MASTER UNDISPUTED REJECTION CHAMPION (2020–2026 STRICT 7-YEAR STUDY)")
print(f"Device: {torch.cuda.get_device_name(0)} | VRAM: {torch.cuda.get_device_properties(0).total_memory / (1024**3):.1f} GB")
print("=" * 135)


def run_2020_onwards_study(is_smoke_test: bool = False):
    t0 = time.time()
    
    # 1. Load Data strictly from 2020-01-01 onwards (with 2019 warmup)
    ...
    # Filter to 2019-12-01 onwards for indicator warmups, trade strictly from 2020-01-01
    df_spot = df_raw[df_raw["day"] >= "2019-12-15"].reset_index(drop=True)
    all_days = sorted(list(df_spot["day"].unique()))
    trading_days_2020 = [d for d in all_days if d >= "2020-01-01"]

    if is_smoke_test:
        trading_days_2020 = trading_days_2020[:5]
        print(f"\n[SMOKE TEST]: Running on first {len(trading_days_2020)} days of 2020 only...")
    else:
        print(f"[1] Loaded {len(trading_days_2020)} Trading Days strictly from 2020-01-01 onwards in {time.time()-t0:.2f}s.")

    # 2. Resample 3m, 5m, 15m
    ...
    # 4. Extract Candidate Signals for 2020 onwards
    candidates = []
    ...
    for day in trading_days_2020:
        ...
        candles_3m_list.extend(d3.to_dict("records"))
        global_offset += len(d3)

        # Update virgin CPR list
        surviving = []
    ...
    print(f"[2] Extracted {len(df_cand):,} 2020-2026 Candidates across {len(candles_3m_list):,} 3m bars.")

    # 5. Build GPU Execution Tensors
    MAX_FUT = 75
    N_SIG = len(df_cand)
    ...
    # 8. All 7 Experiments for 2020-2026
    experiments = [
    ...
    print("\n" + "=" * 135)
    print("EVALUATING STRICT 7-YEAR DATASET (2020–2026) ON GPU...")
    print("=" * 135)
    ...
    print("\n" + "=" * 135)
    print(f"{'EXPERIMENT CONFIGURATION (2020–2026 STRICT)':56s} | {'TRADES':8s} | {'WIN RATE':9s} | {'GREEN DAYS':11s} | {'PF':7s} | {'NET PROFIT (Rs)':18s} | {'CALMAR':8s}")
    print("-" * 135)
    for r in results:
        print(f"{r['name']:56s} | {r['total_trades']:<8,d} | {r['win_rate']:<8.2f}% | {r['daily_win_rate']:<10.1f}% | {r['pf']:<7.3f} | Rs {r['net_rs']:>+14,.2f} | {r['calmar']:<8.2f}")
    print("=" * 135)

    out = ROOT / "artifacts" / "f6_hybrid" / "study_2020_2026_champion_results.json"
...
if __name__ == "__main__":
    ...
    run_2020_onwards_study(is_smoke_test=is_smoke)
