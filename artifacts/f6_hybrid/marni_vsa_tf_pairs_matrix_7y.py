"""
MARNI VSA ENGINE — MULTI-TIMEFRAME BIAS PAIRS MATRIX 7-YEAR BACKTEST (2020 - 2026)
===================================================================================
Compares 6 distinct LTF Execution & HTF Macro Bias pairs:
  1. 1m LTF / 15m HTF (Reference Baseline)
  2. 1m LTF / 5m HTF
  3. 3m LTF / 15m HTF
  4. 3m LTF / 5m HTF
  5. 5m LTF / 15m HTF
...
Specifications for each pair:
  - LTF: Aggregated Candle stream for 3-phase impulse wave detection & Golden Pocket [0.618, 0.786]
  - HTF: Synthesized Heikin-Ashi Closes vs 11-period LinReg SMA Plot + HTF UT Bot (key=1.0, period=10)
  - Target: Official TP = 0.290 Fib Level (0.496x Option Span) | Stop Loss: 1.155 Ext (0.369x Option Span)
  - Friction: ₹15/order Brokerage + 0.50 pt/order Slippage
"""
...
class GeneralHTFBiasState:
    """Configurable HTF Heikin-Ashi + 11-period Linear Regression SMA Plot State."""
...
def process_day(task):
    ...
    htf = GeneralHTFBiasState(period=htf_period, linreg_len=11)
    ut_ltf = UTBotState(key=1.0, period=10)
    ...
    for idx in range(len(spot["min"])):
        ...
        m = row["minute"]
        c_1m = Candle(row["open"], row["high"], row["low"], row["close"], minute=m)
        
        htf.update_1m(c_1m)
        htf_snap = htf.snapshot()

        # Build LTF candle aggregation
        ltf_buf.append(c_1m)
        if len(ltf_buf) == ltf_period:
            ltf_c = Candle(
                open=ltf_buf[0].open,
                high=max(b.high for b in ltf_buf),
                low=min(b.low for b in ltf_buf),
                close=ltf_buf[-1].close,
                minute=m,
            )
            ltf_buf.clear()
            col = ut_ltf.update(ltf_c)
            history.append((ltf_c, col))
        ...
        # Check CE Touches in Pocket [0.618 - 0.786] on 1m Spot
        valid_ce = []
        for s in ce_setups:
            if s.get("triggered", False):
                continue
            pk, orig, sp = s["peak_high"], s["origin_low"], s["span"]
            if c_1m.low < orig - 0.25 * sp:
                continue
            f618 = pk - 0.618 * sp
            f786 = pk - 0.786 * sp
            in_zone = (c_1m.low <= f618 + 0.5) and (c_1m.high >= f786 - 0.5)

            linreg_p = htf_snap.get("linreg_plot")
            ut_col = htf_snap.get("ut_color")

            if in_zone:
                if linreg_p is not None and c_1m.close > linreg_p and ut_col == "green":
                    events.append({
                        "minute": m,
                        "side": "CE",
                        "strike": ce_strike,
        ...
        # Check PE Touches in Pocket [0.618 - 0.786] on 1m Spot
        valid_pe = []
        for s in pe_setups:
            if s.get("triggered", False):
                continue
            orig, pk, sp = s["origin_high"], s["peak_low"], s["span"]
            if c_1m.high > orig + 0.25 * sp:
                continue
            f618 = pk + 0.618 * sp
            f786 = pk + 0.786 * sp
            in_zone = (c_1m.high >= f618 - 0.5) and (c_1m.low <= f786 + 0.5)

            linreg_p = htf_snap.get("linreg_plot")
            ut_col = htf_snap.get("ut_color")

            if in_zone:
                if linreg_p is not None and c_1m.close < linreg_p and (ut_col == "red" or m <= 600):
                    events.append({
                        "minute": m,
                        "side": "PE",
                        "strike": pe_strike,
...
def main():
    parser = argparse.ArgumentParser(description="Marni VSA Multi-Timeframe Bias Pairs 7-Year Backtest")
    ...
    print(f"\n{'='*125}")
    print(f"MARNI VSA ENGINE — MULTI-TIMEFRAME BIAS PAIRS MATRIX 7-YEAR STUDY (2020 - 2026)")
    ...
    for ltf_p, htf_p, pair_label in tf_pairs:
        ...
        st = compute_stats(pair_trades, len(days))
        all_results[pair_key] = {"label": pair_label, "ltf": ltf_p, "htf": htf_p, "stats": st, "trades": pair_trades}
        print(f"Completed {pair_label:45s} in {elapsed:.2f}s | Trades: {st['trades']:5d} | WR: {st['win_rate']:5.1f}% | Net Rs: Rs {st['net_rs']:+12,.2f} | PF: {st['profit_factor']:.2f}")

    print(f"\n{'='*135}")
    print(f"MULTI-TIMEFRAME BIAS PAIRS COMPARATIVE LEADERBOARD ({args.start[:4]} - {args.end[:4]} | TP=0.290)")
    print(f"{'='*135}")
    print(f"{'Timeframe Bias Pair':45s} | {'Trades':6s} | {'Win Rate':9s} | {'Net Points':12s} | {'Profit Factor':14s} | {'Max DD (Rs)':14s} | {'Net Realized P&L (Rs)':22s}")
    print("-" * 140)
    for pk, res in sorted(all_results.items(), key=lambda x: x[1]["stats"]["net_rs"], reverse=True):
        st = res["stats"]
        print(f"{res['label']:45s} | {st['trades']:6d} | {st['win_rate']:8.1f}% | {st['net_points']:+11.2f}p | {st['profit_factor']:13.2f} | Rs {st['max_drawdown_rs']:11,.2f} | Rs {st['net_rs']:+19,.2f}")
    print("-" * 140)

    # Save to json
    out_path = Path("artifacts/f6_hybrid/marni_vsa_tf_pairs_comparison.json")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump({k: {"label": v["label"], "stats": v["stats"]} for k, v in all_results.items()}, f, indent=2)
    print(f"\nSaved timeframe pairs comparison JSON to: {out_path}")
