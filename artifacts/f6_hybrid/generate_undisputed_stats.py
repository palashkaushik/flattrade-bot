"""Generate Comprehensive Granular Metrics for the Undisputed Rejection Champion."""
...
score_mask = (t_sig_custom_scores >= 50)
...
summary = {
    ...
    "total_traded_days": df_t["date"].nunique(),
    "avg_trades_per_day": round(len(df_t) / df_t["date"].nunique(), 2),
    ...
    "win_rate": round(len(df_t[df_t["net_rs"] > 0]) / len(df_t) * 100, 2),
...
Path(ROOT / "artifacts" / "f6_hybrid" / "undisputed_champion_granular_stats.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
