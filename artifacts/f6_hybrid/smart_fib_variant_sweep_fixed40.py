"""Stricter-signal variant sweep on Smart Fib Optimus, fixed Rs40/trade cost.

Compares the champion variant (span15/age45/zone0.5-0.786) against stricter
signal variants V2 (span20), V3 (span20/zone0.618-0.786), V4
(span20/age30/zone0.618-0.786) over targets (0.618, 0.786, 1.0) x stops
(1.05, 1.13, 1.272), fallback 0, threshold 5. Non-WF + expanding WFO.
GPU-only, uses prebuilt tensor caches.
"""
...
VARIANTS = {
    "champion": "12:4:15:45:0.5:0.5:0.786",
    "V2_span20": "12:4:20:45:0.5:0.5:0.786",
    "V3_span20_zone618": "12:4:20:45:0.5:0.618:0.786",
    "V4_span20_age30_zone618": "12:4:20:30:0.5:0.618:0.786",
}


def _params():
    out = []
    for target in (0.618, 0.786, 1.0):
        for stop in (1.05, 1.13, 1.272):
            out.append({
...
def _load(adapter, days, variant, cache_dir, device, batch_size):
    dataset, source, path = grid._cached_variant_dataset(
        adapter.data_root, days, variant, cache_dir,
    )
    if dataset is None:
        raise RuntimeError(f"missing GPU tensor cache for {variant.variant_id}")
    print(f"[CACHE] {variant.variant_id} source={source} events={int(dataset.event_mask.sum())}", flush=True)
...
def main(argv=None):
    ...
    parser.add_argument("--output", default="artifacts/f6_hybrid/smart_fib_variant_sweep_fixed40.json")
    ...
    report = {
        "engine": "smart_fib_variant_sweep_fixed40",
        "mode": "stricter_signal_variant_sweep_fixed40",
    ...
    print("=== FULL WINDOW (non-WF): best config per variant ===", flush=True)
    for label, spec in VARIANTS.items():
        variant = grid._parse_variant(spec)
        dataset, resident, evaluator = _load(adapter, days, variant, cache_dir, device, args.batch_size)
        ...
        print(
            f"  {label}: #1 t={ranked[0]['params']['target_level']} s={ranked[0]['params']['stop_level']} "
            f"trades={ranked[0]['trades']} WR={ranked[0]['win_rate']:.2f} net={ranked[0]['net_points']:+.2f} "
            f"net_rs={ranked[0]['net_rs']:+,.2f} DD={ranked[0]['max_drawdown_points']:.2f} PF={ranked[0]['profit_factor']:.4f}",
            flush=True,
        )
        ...
        for fold_index, (_, train_end, val_start, val_end) in enumerate(FOLDS):
            ...
            validation = dict(raw_validation, params=winner["params"])
            ...
            print(
                f"    [WF] fold={fold_index + 1} train t={winner['params']['target_level']}/"
                f"s={winner['params']['stop_level']} score={winner['train_score']:+.2f} | "
                f"val trades={validation['trades']} net={validation['net_points']:+.2f} "
        ...
        print(
            f"  {label} STITCHED: trades={stitched['trades']} WR={stitched['win_rate']:.2f} "
        ...
        report["variants"][label] = {
            ...
            "events": int(dataset.event_mask.sum()),
            "full_window_ranked": [dict(result) for result in ranked],
            "folds": fold_results,
