"""High-Yield Mega-Runner with 15-Minute Theta Cut on August 18, 19, 20."""
...
from artifacts.f6_hybrid.test_super_only_aug import extend_with_august, ParamStoch, bslice, to_hhmm, LOT_SIZE, FEE
from artifacts.f6_hybrid.compare_rules_1_and_2 import load_full_ohlc_spot
from artifacts.f6_hybrid.run_high_yield_aug import HighYieldDetector
import grid_optimize_f6_atr as grid

def simulate_hybrid_high_yield(
    day: str, opt_map: dict, all_cal: list, cal_idx: dict, spot_all: dict,
    initial_sl_pts: float = 6.0,
    lock_trigger_pts: float = 10.0,
    locked_profit_pts: float = 8.0,
    ...
    for minute in range(560, 931):
        if pos is not None:
            ...
            if held:
                ...
                # 15-Min Theta Cut
                if pos["duration_min"] >= time_stop_min and not pos["is_locked"]:
                    theta_sl = pos["entry"] - 2.50
                    if theta_sl > pos["sl"]:
                        pos["sl"] = round(theta_sl, 2)
                        pos["is_theta_cut"] = True

                # High-Yield Lock at +10 pts -> Lock +8 pts
                ...
                if l <= pos["sl"] and h >= pos["tp"]:
                    ex, rsn = pos["sl"], "PROFIT_LOCK" if pos.get("is_locked") else ("THETA_CUT" if pos.get("is_theta_cut") else "SL")
                elif h >= pos["tp"]:
            ...
            if ai and ai[2] == sig_stk and pos is None:
                ...
                if bar:
                    ...
                    pos = {
                        ...
                        "duration_min": 0, "is_locked": False, "is_trailing": False, "is_theta_cut": False,
                        "stype": stype,
                    }
                    trades_today += 1
                    break
...
def main():
    ...
    print("=" * 135)
    print("HIGH-YIELD ASYMMETRIC RUNNER + 15-MIN THETA CUT: AUGUST 18, 19, 20, 2026")
    print("Rule: SL = -6.0 pts | Lock +8.0 pts at +10.0 pts gain | Trail = 4.0 pts | 15m Theta Cut (-2.5 pts)")
    print("=" * 135)

    all_trs = []
    for day in ["2026-08-18", "2026-08-19", "2026-08-20"]:
        trs = simulate_hybrid_high_yield(day, opt_map, all_cal, cal_idx, spot_all)
        all_trs.extend(trs)
