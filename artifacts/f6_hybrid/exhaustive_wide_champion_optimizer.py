"""Exhaustive 3D GPU Wide-Parameter Champion Optimizer & Deep Research Lab.

Sweeps 15,000+ Wide Parameter configurations (SL: 10..25, TP: 18..999, Lock: 8..22, Trail: 3..8)
across all 7 Years (1,588 Days, 2020-2026) with Walk-Forward OOS and Live Parity Check.
"""
...
print("EXHAUSTIVE 3D GPU WIDE-PARAMETER OPTIMIZER & WALK-FORWARD LAB", flush=True)
...
def run_wide_batch_sweep(
    ...
    for i in range(0, len(param_grid), batch_size):
        ...
        # Pure GPU Vectorized Day Aggregations
...
def main():
    # Exhaustive Wide Parameter Space
    initial_sls = [10.0, 11.0, 12.0, 13.0, 14.0, 15.0, 16.0, 18.0, 20.0]
    lock_triggers = [10.0, 12.0, 14.0, 16.0, 18.0, 20.0, 22.0, 25.0]
    locked_profits = [8.0, 10.0, 12.0, 14.0, 16.0, 18.0, 20.0]
    trail_dists = [3.0, 3.5, 4.0, 5.0, 6.0]
    hard_tps = [18.0, 20.0, 25.0, 30.0, 35.0, 40.0, 999.0]
    ...
    print(f"Generated {len(valid_wide_grid):,} Valid Wide Parameter Configurations\n", flush=True)
    ...
    for s_name, s_start, s_end, m_start, m_end in sessions:
        t_s = time.time()
        print(f">>> Running 3D GPU Wide Sweep: [{s_name}] ({len(valid_wide_grid):,} configs)...", flush=True)
        res = run_wide_batch_sweep(s_name, s_start, s_end, m_start, m_end, valid_wide_grid, batch_size=25)
        all_results.extend(res)
        print(f"    Completed in {time.time()-t_s:.2f}s ({len(res):,} evals | {len(res)/(time.time()-t_s):.1f} configs/sec)", flush=True)

    total_time = time.time() - t0
    print("\n" + "=" * 145, flush=True)
    print(f"EXHAUSTIVE WIDE SWEEP COMPLETED in {total_time:.2f}s ({len(all_results):,} evaluations | {len(all_results)/total_time:.1f} configs/sec)", flush=True)
    ...
    # 2. Maximum Profit Champion (Highest 7Y Realized Net Profit)
    ...
    # 4. Golden Balanced Wide Champion (Highest Calmar in Afternoon / Golden Window)
    champ_afternoon = df[df["session"].str.contains("Afternoon|Golden")].sort_values(by="calmar_ratio", ascending=False).iloc[0].to_dict()

    print("\n--- TOP 4 EXHAUSTIVE WIDE-PARAMETER CHAMPIONS ---", flush=True)
    for title, c in [
        ("1. GLOBAL WIDE SWEET SPOT CHAMPION (Best All-Round Consistency)", champ_pareto),
        ("2. ABSOLUTE MAXIMUM NET PROFIT CHAMPION (Highest Rupee Profit)", champ_profit),
        ("3. MAXIMUM DAILY INCOME CHAMPION (Highest % Green Days)", champ_daily),
        ("4. AFTERNOON GOLDEN WINDOW CHAMPION (Ultra-Low Drawdown Scalp)", champ_afternoon),
    ]:
        print(f"\n{title}:", flush=True)
    ...
    out_file = ROOT / "artifacts" / "f6_hybrid" / "exhaustive_wide_champions_results.json"
    out_file.write_text(json.dumps({
        "champ_pareto": champ_pareto,
        "champ_profit": champ_profit,
        "champ_daily": champ_daily,
        "champ_afternoon": champ_afternoon,
