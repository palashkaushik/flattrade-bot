"""Optimus Morning Champion GPU Optimizer & Morning/Afternoon Twin-Engine Lab.

Exhaustively sweeps all Morning sub-windows (09:15-10:00, 09:30-10:15, 09:30-10:45, 09:30-11:30, 10:00-11:30, 09:30-12:30)
across 10,000+ wide & fast geometry configurations on RTX 3060.
Fuses the winning Morning Champion with the winning Afternoon Champion.
"""
...
print("OPTIMUS MORNING CHAMPION GPU OPTIMIZER (RTX 3060)", flush=True)
...
def main():
    # Fine-Grained Morning Geometry Grid
    initial_sls = [6.0, 8.0, 10.0, 12.0, 14.0, 16.0]
    lock_triggers = [8.0, 10.0, 12.0, 14.0, 16.0, 18.0, 20.0]
    locked_profits = [6.0, 8.0, 10.0, 12.0, 14.0, 16.0]
    trail_dists = [2.5, 3.0, 4.0, 5.0]
    ...
    for sl, l_trig, l_prof, trail, tp in itertools.product(initial_sls, lock_triggers, locked_profits, trail_dists, hard_tps):
        ...
        if l_prof > tp:
            continue
        valid_morning_grid.append({
            "initial_sl": sl, "lock_trigger": l_trig, "locked_profit": l_prof,
            "trail_dist": trail, "hard_tp": tp,
        })

    print(f"Generated {len(valid_morning_grid):,} Valid Morning Parameter Sets\n", flush=True)

    # Morning Sub-Sessions (09:15 = bar 0, 09:30 = bar 15, 10:00 = bar 45, 10:15 = bar 60, 10:45 = bar 90, 11:30 = bar 135, 12:30 = bar 195)
    morning_sessions = [
        ("1. Opening Bell Sweet Spot (09:30-10:15)", 15, 60),
        ("2. Opening Momentum Expansion (09:30-10:45)", 15, 90),
        ("3. Early Open Surge (09:15-10:30)", 0, 75),
        ("4. Mid-Morning Institutional (10:00-11:30)", 45, 135),
        ("5. Extended Morning Half (09:30-11:30)", 15, 135),
        ("6. Complete Morning Session (09:30-12:30)", 15, 195),
        ("7. Pre-Midday Expansion (10:15-12:30)", 60, 195),
    ]

    all_results = []
    t0 = time.time()

    for s_name, s_start, s_end in morning_sessions:
        t_s = time.time()
        print(f">>> Evaluating Morning Window: [{s_name}]...", flush=True)
        res = run_morning_sub_sweep(s_name, s_start, s_end, valid_morning_grid, batch_size=25)
    ...
    print("\n" + "=" * 145, flush=True)
    print(f"ALL MORNING SESSIONS COMPLETED in {total_time:.2f}s ({total_evals:,} evaluations | {total_evals/total_time:.1f} configs/sec)", flush=True)
    print("=" * 145, flush=True)

    df = pd.DataFrame(all_results)

    # Rank Morning Sessions
    print("\n" + "=" * 145, flush=True)
    print("ALL MORNING SESSIONS RANKED BY PERFORMANCE (BEST CONFIG PER WINDOW)", flush=True)
    print("=" * 145, flush=True)
    best_per_morning = df.sort_values(by="calmar_ratio", ascending=False).groupby("session").first().reset_index()
    best_per_morning = best_per_morning.sort_values(by="net_rs", ascending=False)
    print(best_per_morning[["session", "trades", "daily_win_rate", "trade_win_rate", "net_points", "net_rs", "profit_factor", "max_drawdown", "calmar_ratio", "month_win_rate"]].to_string(index=False), flush=True)

    # Morning Champion
    champ_morning = best_per_morning.iloc[0].to_dict()

    print("\n" + "=" * 145, flush=True)
    print("🏆 THE ABSOLUTE MORNING CHAMPION", flush=True)
    print("=" * 145, flush=True)
    c = champ_morning
    print(f"  * Morning Window:             {c['session']}", flush=True)
    ...
    out_file = ROOT / "artifacts" / "f6_hybrid" / "morning_champion_results.json"
    out_file.write_text(json.dumps({
        "champ_morning": champ_morning,
        "best_per_morning": best_per_morning.to_dict(orient="records"),
        "top_50": df.sort_values(by="calmar_ratio", ascending=False).head(50).to_dict(orient="records"),
    }, indent=2, default=float), encoding="utf-8")
    print(f"\n[Saved Morning Champions Results JSON]: {out_file}", flush=True)
