"""F6+SUPER+REVERSAL — fixed variable shadowing, 60-bar warmup, causal."""
...
OUT=Path(__file__).with_name("f6_super_reversal_fixed_result.json")
...
if __name__=="__main__":
    ...
    if all_trades:
        ...
        print(tdf["reason"].value_counts().to_string())
    import json
    out=Path(__file__).with_name(f"f6_super_reversal_fixed_mode{args.mode}_result.json")
