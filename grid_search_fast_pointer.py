"""Ultra-Fast Incremental Pointer-Based SL/TP Grid Search Engine.

Architecture:
  1. Incremental Pre-computation: Generates candidate entry signals for each timeframe ONCE per day.
  2. Pointer-Based Vectorized Simulation: Tests 24 SL/TP combinations over pre-computed signals in seconds.
"""
...
from backtest_5y_optimized import load_spot, option_files, init_worker, SYM_RE, to_minutes, latest_spot, DualTimeframeSymbolTracker, TimeframeTracker, summarize, CE_OFFSET, PE_OFFSET, SESSION_START, SESSION_END, DAY_LAST, DAILY_SHUTDOWN_LOSS_RS, DAILY_SHUTDOWN_PROFIT_RS, CONSECUTIVE_LOSS_LIMIT, LOT_SIZE
...
def precompute_day_signals(args):
    """Incremental Pre-computation: Computes option slices and per-minute signals ONCE per day."""
...
def simulate_day_fast(day_data, tf_label, sl_pts, tp_pts):
    """Fast pointer-based simulation over pre-computed day signals."""
    if not day_data:
        return []
    ...
    per_minute_triggers = day_data["per_tf_triggers"][tf_label]
        ...
        if pos is not None:
            ...
            if held is not None:
                ...
                if daily_pnl + (c_px - pos["entry"]) * LOT_SIZE <= -DAILY_SHUTDOWN_LOSS_RS:
                    ...
                    trades.append({
                        "date": pos["date"], "entry_min": pos["entry_min"], "exit_min": minute,
        ...
        for (signal_side, signal_strike, signal_symbol, c_px, is_rev) in trigs:
            ...
            if active_info and active_info[2] == signal_strike and pos is None:
                ...
                if bar is not None:
                    ...
                    pos = {
                        "date": "", "side": actual_side, "symbol": actual_symbol, "slice": actual_slice,
...
def main():
    ...
    print(f"Step 1: Incremental Pre-computation of candidate signals across {len(days)} days...")
    t0 = time.time()
    ...
    t_pre = time.time() - t0
    print(f"[OK] Incremental Pre-computation finished in {t_pre:.2f} seconds!")

    sl_range = [6.0, 8.0, 10.0, 12.0]
    tp_range = [12.0, 15.0, 18.0, 20.0, 25.0, 30.0]
    tf_labels = ["1m", "2m", "3m", "5m"]

    print(f"\nStep 2: Microsecond Pointer-Based Grid Search Sweeping {len(sl_range)*len(tp_range)*len(tf_labels)} Combinations...")
    t1 = time.time()

    all_grid_results = []

    for tf_label in tf_labels:
        print(f"\n--- GRID SEARCH FOR {tf_label.upper()} TIMEFRAME ---")
        for sl, tp in product(sl_range, tp_range):
            ...
            all_grid_results.append({
                "tf": tf_label, "sl": sl, "tp": tp, "rr": rr,
                "trades": st["trades"], "wr": st["wr"], "pts": st["pts"], "rs": st["rs"], "pf": st["pf"]
            })
            print(f"  {tf_label:5s} | SL: {sl:4.1f} | TP: {tp:4.1f} (R:R 1:{rr:<4.2f}) -> Net Profit: Rs {st['rs']:+9,d} | WR: {st['wr']:5.1f}% | PF: {st['pf']:.2f}")

    t_sim = time.time() - t1
    print(f"\n[OK] COMPLETED FULL GRID SEARCH SIMULATION IN {t_sim:.2f} SECONDS!")

    df_all = pd.DataFrame(all_grid_results)

    print("\n" + "=" * 125)
    print("BEST SL / TP RISK-REWARD SETTING FOR EACH TIMEFRAME RANKED BY NET PROFIT")
    print("=" * 125)
