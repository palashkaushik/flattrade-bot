...
# ---------------------------------------------------------------------------
# 4. Sweep definition — champion ± axes, seeded-mode-specific
# ---------------------------------------------------------------------------
CHAMPION = dict(kind='B', arm_window=10, use_elder=False, use_rsi=False,
                reversal=False, atr_sl=True, atr_mult=1.5, atr_period=10,
                cap=0, use_bias=False, be_trigger=0.70, be_buffer=1.0,
                tp_frac=1.0, touch_buffer=0.0, entry_start=0)

grid = []
if SMOKE:
    # Tiny validation grid: 6 configs, mixed axes
    for arm, atr_p, atr_m, tb, be_t in [(10, 10, 1.5, 0.0, 0.70), (10, 14, 1.5, 0.0, 0.70),
                                        (5, 10, 2.0, 0.5, 0.50), (15, 14, 2.5, 1.0, 0.70),
                                        (10, 10, 1.5, 0.5, 0.50), (20, 10, 2.0, 0.0, 0.50)]:
        cfg = dict(CHAMPION)
        cfg.update(arm_window=arm, atr_period=atr_p, atr_mult=atr_m,
                   touch_buffer=tb, be_trigger=be_t, be_buffer=1.0,
                   entry_end=T1, entry_start=0)
        grid.append(cfg)
else:
    ...
    # Morning-block axis (seeded mode's weakness: warm morning multi-TF noise).
    for arm in [10, 15]:
...
# ---------------------------------------------------------------------------
# 5. Metrics: net, WR, equity maxDD, WORST-DAY, daily-dd
# ---------------------------------------------------------------------------
def metrics_from_trades(trades, lot=LOT, fee=FEE):
    if not trades:
...
# ---------------------------------------------------------------------------
# 6. Run in chunks per touch-buffer (bounce stacks are per-buffer)
# ---------------------------------------------------------------------------
results = []
t_sweep = time.time()
for tb in TOUCH_BUFFERS_SWEEP:
    # Point the engine's bounce selection at this buffer's stack:
    G.bounce_pe_stack = torch.stack([bounce_pe_seeds[tb]] * len(G.TOUCH_BUFFERS), 0)
    G.bounce_ce_stack = torch.stack([bounce_ce_seeds[tb]] * len(G.TOUCH_BUFFERS), 0)
    ...
    for i in range(0, len(cfgs), CHUNK):
        ...
        print(f"  [tb={tb}] {i + len(batch)}/{len(cfgs)} done ({time.time()-t_sweep:.0f}s)")

print(f"[sweep] complete in {time.time()-t_sweep:.0f}s")

# ---------------------------------------------------------------------------
# 7. Report: rank by net with drawdown guards
# ---------------------------------------------------------------------------
def fmt(cfg, m, rank):
    return (f"#{rank:3d} net=Rs {m['net']:>12,.0f} | dd=Rs {m['max_dd']:>9,.0f} | worst-day=Rs {m['worst_day']:>9,.0f} | "
            f"calmar={m['calmar']:>6.1f} | wr={m['wr']:>5.1f}% | trades={m['trades']:>6} | "
            f"arm={cfg['arm_window']:>2} atr={cfg['atr_period']}/{cfg['atr_mult']} tb={cfg['touch_buffer']} "
            f"be={cfg['be_trigger']} es={cfg.get('entry_start', 0)}")

# Rank A: max net (with min-trades floor to avoid degenerate configs)
valid = [(c, m) for c, m in results if m['trades'] >= 1000]
by_net = sorted(valid, key=lambda x: -x[1]['net'])
by_calmar = sorted([r for r in valid if r[1]['net'] > 0], key=lambda x: -x[1]['calmar'])

print("\n================= SEEDED SWEEP — TOP 20 BY NET =================")
for i, (c, m) in enumerate(by_net[:20]):
    print(fmt(c, m, i + 1))

print("\n================= TOP 20 BY CALMAR (net>0) =====================")
for i, (c, m) in enumerate(by_calmar[:20]):
    print(fmt(c, m, i + 1))

# Champion A/B in seeded mode (the live-equivalent behavior)
champ_seeded = [r for r in results
                if r[0]['arm_window'] == 10 and r[0]['atr_period'] == 10
                and r[0]['atr_mult'] == 1.5 and r[0]['touch_buffer'] == 0.0
                and r[0]['be_trigger'] == 0.70 and r[0].get('entry_start', 0) == 0]
print("\n================= CHAMPION (seeded mode) A/B ====================")
for c, m in champ_seeded:
    print(fmt(c, m, 0))

# Persist full results
import csv
with open('sweep_seeded_results.csv', 'w', newline='') as f:
    w = csv.writer(f)
    w.writerow(['arm_window', 'atr_period', 'atr_mult', 'touch_buffer', 'be_trigger',
                'entry_start', 'trades', 'wr', 'net', 'max_dd', 'worst_day', 'calmar'])
    for c, m in results:
        w.writerow([c['arm_window'], c['atr_period'], c['atr_mult'], c['touch_buffer'],
                    c['be_trigger'], c.get('entry_start', 0), m['trades'],
                    round(m['wr'], 2), round(m['net'], 2), round(m['max_dd'], 2),
                    round(m['worst_day'], 2), round(m['calmar'], 3)])
print("\n[saved] sweep_seeded_results.csv")
