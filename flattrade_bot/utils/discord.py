...
class DiscordNotifier:
    ...
    async def notify_trade_open(self, trade_info: Dict[str, Any]):
        ...
        embed = {
            "title": f"🚀 TWO-BAR CONFIRMATION TRIGGER: {symbol}",
            "color": 0x2ECC71,  # Green
            "fields": [
                {"name": "Strategy", "value": "🏆 Undisputed Rejection Champion", "inline": True},
                {"name": "Option Strike", "value": symbol, "inline": True},
                {"name": "Direction", "value": f"**{direction}**", "inline": True},
                {"name": "S/R Level", "value": level, "inline": True},
                {"name": "Confluence Score", "value": f"**{score} pts**", "inline": True},
    ...
    async def notify_trailing_sl_updated(self, trail_info: Dict[str, Any]):
        """Alerts when the trailing stop moves up/down to lock in profit."""
        symbol = trail_info.get("symbol", "NIFTY")
        new_sl = trail_info.get("new_sl", 0.0)
        gain_pts = trail_info.get("gain_pts", 0.0)

        embed = {
            "title": f"🛡️ TRAILING STOP LOCKED: {symbol}",
            "color": 0x3498DB,  # Blue
            "fields": [
                {"name": "Strategy", "value": "Undisputed Rejection Champion", "inline": True},
                {"name": "Current Gain", "value": f"**+{gain_pts:.2f} pts**", "inline": True},
                {"name": "New Protected SL", "value": f"**₹{new_sl:.2f}**", "inline": True},
    ...
    async def notify_trade_close(self, trade_info: Dict[str, Any]):
