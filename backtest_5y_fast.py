"""Ultra-Fast Multi-Core 5-Year Backtest Engine for Quad Rotation Strategy.

Optimizations:
  1. Shared memory spot dictionary per worker process.
  2. C-engine fast CSV reader.
  3. ITM2 active strike filtering.
  4. Complete 5-Year (2020-2024) execution in seconds.
"""
...
def load_day_options(path: Path):
    try:
        df = pd.read_csv(path, usecols=["date", "time", "symbol", "open", "high", "low", "close"], engine="c")
        if df.empty:
            return None, {}, None
...
def process_single_day(args):
    ...
    prev_file_path = Path(prev_file_path_str) if prev_file_path_str else None

    spot = GLOBAL_SPOT.get(day)
    if spot is None or not file_path.exists():
        return []

    df_curr, groups_curr, prefix = load_day_options(file_path)
    if prefix is None or not groups_curr:
        return []

    groups_prev = {}
    if prev_file_path and prev_file_path.exists():
        _, groups_prev, _ = load_day_options(prev_file_path)

    sym_trackers = {}
    for sym, g in groups_prev.items():
        sym_trackers[sym] = SymbolTracker1M()
        mins = g["min"].to_numpy()
        ...
        for i in range(len(mins)):
            sym_trackers[sym].push_1m(Candle(open=opens[i], high=highs[i], low=lows[i], close=closes[i], minute=mins[i]))

    per_minute_triggers = {}
    for sym, g in groups_curr.items():
        if sym not in sym_trackers:
            sym_trackers[sym] = SymbolTracker1M()
        tracker = sym_trackers[sym]
        ...
        for i in range(len(mins)):
            ...
            if t1:
                per_minute_triggers.setdefault(m, []).append((side_val, strike_val, sym, closes[i], is_rev))
    ...
    def get_slice(side, minute):
        ...
        if symbol not in slices:
            slices[symbol] = make_slice(groups_curr, symbol)
...
def main():
    parser = argparse.ArgumentParser(description="Ultra-Fast Multi-Core Parallelized 5-Year Backtest Engine")
